    ...
    class QRScanPopup extends AbstractAwaitablePopup {
        ...
        mounted() {
            ...
            navigator.mediaDevices.getUserMedia({audio: false, video: true}).then( () => {
                ...
                const video_devices = _.filter(devices, d => d.kind === "videoinput");
                let deviceId = false;
                let facingMode = false;

                if (video_devices.some( device => !device.deviceId )) {
                    this.showPopup('ErrorPopup', {
                        body: "Browser returns devices empty device IDs. Perhaps you need to use https connection?",
                    });
                    return;
                }
                ...
                if (active_camera_id && this.video_devices_by_id[active_camera_id]) {
